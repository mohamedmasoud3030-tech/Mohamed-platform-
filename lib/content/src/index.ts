 ...
 *
 * This package is the single authority for bilingual (AR/EN) support content
 * that more than one artifact consumes: the public site renders it and the API
 * grounds the LENA Assistant on it. It must stay dependency-free (no React, no
 * Node APIs) so both the browser bundle and the server can import it.
 *
 * Content rules (moved with the content, they travel together):
 ...
 * - Keep it short: this is not a help centre, it is the questions people ask.
 */
...
export type HelpArticle = {
  ...
  link?: { to: string; label: Record<SupportedLocale, string> };
};

export const HELP_TOPICS: Array<{ id: HelpTopicId; label: Record<SupportedLocale, string> }> = [
...
export function searchArticles(query: string, locale: SupportedLocale): HelpArticle[] {
...
export function faqJsonLd(locale: SupportedLocale): Record<string, unknown> {
  return {
